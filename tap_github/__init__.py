...
KEY_PROPERTIES = {
    ...
    'releases': ['id'],
    'repositories': ['id'],
    'community_profile': ['_sdc_repository'],
    'reviews': ['id'],
    'review_comments': ['id'],
    'pr_commits': ['id'],
    'events': ['id'],
...
def get_community_profile(schemas, repo_path, state, mdata, _start_date):
    '''
    Fetch a single record for the community profile about the repo at repo_path. This only
    loads a single record, not a list. It will work off the list of repos defined
    in the config file.
    '''

    with metrics.record_counter('community_profile') as counter:
        for response in authed_get_all_pages(
                'community_profile',
                'https://api.github.com/repos/{}/community/profile'.format(repo_path)
        ):
            r = response.json()
            extraction_time = singer.utils.now()
            r['_sdc_repository'] = repo_path
            ...
            with singer.Transformer() as transformer:
                rec = transformer.transform(r, schemas, metadata=metadata.to_map(mdata))
            singer.write_record('community_profile', rec, time_extracted=extraction_time)
            singer.write_bookmark(state, repo_path, 'community_profile', {'since': singer.utils.strftime(extraction_time)})
            counter.increment()
